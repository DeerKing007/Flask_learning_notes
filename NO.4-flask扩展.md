...
```python
import os
#项目目录下的app下的static下的media
app.config['MEDIA_DIR']=os.path.join(os.path.abspath(os.path.dirname(__file__)),"static/media/")
```
...
def upup():
    ...
    #接收上传来的文件
    file = request.files.get("source") #file.filename file.content_type
    ...
    file.save(os.path.join(app.config['MEDIA_DIR'], uniquefilename))  # 将文件存入指定目录位置
